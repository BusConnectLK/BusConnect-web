...
import { setAdminPilotStatus, ApiError } from "@/lib/api";

export function StatusActions({ pilotId, status }: { pilotId: string; status: string }) {
  ...
  async function setStatus(next: "active" | "rejected") {
    ...
    try {
      ...
      if (!session) return;
      await setAdminPilotStatus(session.access_token, pilotId, next);
      router.refresh();
    } catch (e) {
